...
int sleepTimeMicroseconds = 5000;
// 100000
...
void processList()
{
    ...
    usleep(sleepTimeMicroseconds * 1000);
}
...
void executeMove(MoveInstruction instruction, string currentInstructionText, string nextInstructionText)
{
    ...
    for (int i = 0; i < instruction.Quantity; i++)
    {
        ...
        char boxLetter = _craneHandler.GetTopBoxLetterInColumn(instruction.SourceColumn);
        ...
        _craneHandler.SetCraneDestinationY(yPositionToMoveTo - 1);

        _craneHandler.AnimateMoveVertically(sleepTimeMicroseconds, currentInstructionText, nextInstructionText);

        _craneHandler.ReleaseBox();
