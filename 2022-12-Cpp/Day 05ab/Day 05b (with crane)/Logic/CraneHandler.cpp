...
{
    _crane.DestinationYPosition = _crane.TallestBoxHeight - _crane.BoxLettersGrabbed.size() - 4;
...
void CraneHandler::DrawCraneOnCratesAscii()
{
    ...
    if (_crane.AreBoxesGrabbed)
    {
        for (int i = 0; i < _crane.BoxLettersGrabbed.size(); i++)
        {
            DrawBoxInConsole(CharListHelpers::GetListEntryAt(_crane.BoxLettersGrabbed, i), _crane.CurrentXPosition, _crane.CurrentYPosition + i + 1);
        }
    }
}
...
void CraneHandler::EraseCraneOnCratesAscii()
{
    ...
    if (_crane.AreBoxesGrabbed){
        for (int i = 0; i < _crane.BoxLettersGrabbed.size(); i++)
        {
            EraseBoxInConsole(_crane.CurrentXPosition, _crane.CurrentYPosition + i + 1);
        }
    }
}
...
void CraneHandler::DrawBoxInConsole(char boxLetter, int x, int y)
{
    _consoleHelpers.DrawCharacterAt('[', x - 1, y);
    _consoleHelpers.DrawCharacterAt(boxLetter, x, y);
    _consoleHelpers.DrawCharacterAt(']', x + 1, y);
}
...
{
    _consoleHelpers.DrawCharacterAt(' ', x - 1, y);
    _consoleHelpers.DrawCharacterAt(' ', x, y);
    _consoleHelpers.DrawCharacterAt(' ', x + 1, y);
}

void CraneHandler::GrabBoxes(int numberOfBoxesToGrab)
{
    _crane.AreBoxesGrabbed = true;

    for (int i = 0; i < numberOfBoxesToGrab; i++)
    {
        string currentLine = StringListHelpers::GetListEntryAt(_cratesAscii, _crane.CurrentYPosition + i + 1);

        _crane.BoxLettersGrabbed.push_back(currentLine[_crane.CurrentXPosition]);
        ...
        _cratesAscii = StringListHelpers::ReplaceEntryAt(_cratesAscii, _crane.CurrentYPosition + i + 1, currentLine);
    }
}

void CraneHandler::ReleaseBoxes()
{
    _crane.AreBoxesGrabbed = false;
    ...
    for (int i = 0; i < _crane.BoxLettersGrabbed.size(); i++)
    {
        ...
        currentLine[_crane.CurrentXPosition - 1] = '[';
        currentLine[_crane.CurrentXPosition] = CharListHelpers::GetListEntryAt(_crane.BoxLettersGrabbed, i);
        currentLine[_crane.CurrentXPosition + 1] = ']';

        _cratesAscii = StringListHelpers::ReplaceEntryAt(_cratesAscii, _crane.CurrentYPosition + i + 1, currentLine);
    }

    ClearBoxesGrabbedList();
}
...
list<char> CraneHandler::GetBoxLettersToPickUp(int column, int numberOfBoxesToMove)
{
    int xPosition = CalculateColumnXPosition(column);

    list<char> returnCharacters;
    ...
    for (int i = 0; i < _cratesAscii.size(); i++)
    {
        ...
        {
            returnCharacters.push_back(currentCharacterFound);
...
void CraneHandler::ClearBoxesGrabbedList()
{
    _crane.BoxLettersGrabbed.resize(0);
}